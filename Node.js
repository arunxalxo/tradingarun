...
if (!DRY_RUN && !UPSTOX_ACCESS_TOKEN) {
  console.error('UPSTOX_ACCESS_TOKEN required for live mode. Set DRY_RUN=true to test without it.');
}
...
async function placeOrder({ action, qty, instrumentToken }) {
  if (DRY_RUN) {
    const simulated = { status: 'DRY_RUN', data: { order_id: `DRY_${Date.now()}`, instrument_token: instrumentToken, transaction_type: action, quantity: qty, average_price: null } };
...
app.post('/tv-webhook', async (req, res) => {
  try {
    ...
    if (payload.oi && payload.oi !== 'NA') {
      ...
      if (!Number.isNaN(oiVal) && oiVal <= 0) throw new Error('Rejected: Open Interest too low');
    }
...
// SERVER
app.listen(PORT, () => console.log(`Upstox Bridge running on ${PORT} (DRY_RUN=${DRY_RUN})`));
